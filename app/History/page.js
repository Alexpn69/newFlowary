...
export default function History() {
  ...
  const { isLoading, arrayBlock, notif } = useGetAllLogs();
    ...
    return (
      <>
        {arrayBlock && (
          <div>
            <div>
              Smotri v consoli arrayBlock - eto statistika po vsem strimam, nizhe vivel info po 1
              strimu dly obrazca
            </div>
            ...
            <div>
              {' '}
              Vremya nachala strima {dayjs.unix(arrayBlock[0]?.startAt).format('HH:mm DD/MM/YYYY')}
            </div>
            <div>
              Vremya finisha strima {dayjs.unix(arrayBlock[0]?.time).format('HH:mm DD/MM/YYYY')}
            </div>
