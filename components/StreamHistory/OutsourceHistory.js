...
export const OutsourceHistory = ({ outsource, symbolToken }) => {
  const { currentTx, pagPages } = usePagination({ inArr: outsource });
  console.log("ouyttt", currentTx);

  const statusOutsource = {
    1: "Waiting for claim",
    2: "Waiting for accept",
    3: "Finished",
  };
  return (
    <>
      <ul className={styles.list}>
        ...
        <li>
          {currentTx.length != 0 ? (
            currentTx.map(
              ({ who, status, taskName, wage, startDate, deadline }) => (
                <ul className={styles.outsourcerow} key={startDate}>
                  ...
                  <li className={clsx(styles[`status${status}`])}>
                    {statusOutsource[status]}
                  </li>
